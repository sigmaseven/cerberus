...
// TestSQLite_Close_ComprehensiveVersion tests graceful shutdown (comprehensive version)
func TestSQLite_Close_ComprehensiveVersion(t *testing.T) {
	logger := zap.NewNop().Sugar()
	sqlite, err := NewSQLite(":memory:", logger)
	require.NoError(t, err)
...
// createTestSigmaRule creates a valid SIGMA rule with SigmaYAML for testing
func createTestSigmaRule(id, name string) *core.Rule {
	return &core.Rule{
		ID:          id,
		Name:        name,
...
func TestSQLiteRuleStorage_SaveRule(t *testing.T) {
	...
	rule := createTestSigmaRule(uuid.New().String(), "Test Rule")
...
func TestSQLiteRuleStorage_GetRuleByID(t *testing.T) {
	...
	rule := createTestSigmaRule(ruleID, "Retrieval Test")
...
// TestSQLiteRuleStorage_UpdateRule_Comprehensive tests rule updates (comprehensive version)
func TestSQLiteRuleStorage_UpdateRule_Comprehensive(t *testing.T) {
	_, ruleStorage, cleanup := setupTestSQLiteWithRules(t)
	if ruleStorage == nil {
		return
	}
	...
	// Create initial rule
	ruleID := uuid.New().String()
	rule := createTestSigmaRule(ruleID, "Original Name")
	rule.Severity = "Low"
	...
	// Update the rule
	updatedRule := createTestSigmaRule(ruleID, "Updated Name")
	updatedRule.Severity = "High"
...
// TestSQLiteRuleStorage_DeleteRule_Comprehensive tests rule deletion (comprehensive version)
func TestSQLiteRuleStorage_DeleteRule_Comprehensive(t *testing.T) {
	_, ruleStorage, cleanup := setupTestSQLiteWithRules(t)
	if ruleStorage == nil {
		return
	}
	...
	// Create a rule
	ruleID := uuid.New().String()
	rule := createTestSigmaRule(ruleID, "To Be Deleted")
...
func TestSQLiteRuleStorage_ListRules(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		rule := createTestSigmaRule(uuid.New().String(), "Test Rule "+string(rune('A'+i)))
		err := ruleStorage.CreateRule(rule)
		require.NoError(t, err)
	}
...
func TestSQLiteRuleStorage_ListRules_Pagination(t *testing.T) {
	...
	// Create 10 rules
	for i := 0; i < 10; i++ {
		rule := createTestSigmaRule(uuid.New().String(), "Pagination Test "+string(rune('0'+i)))
		err := ruleStorage.CreateRule(rule)
		require.NoError(t, err)
	}
...
func TestSQLite_PreparedStatements_Comprehensive(t *testing.T) {
	logger := zap.NewNop().Sugar()
	sqlite, err := NewSQLite(":memory:", logger)
	require.NoError(t, err)
	defer sqlite.Close()
